...
If I rewrite the code above and plot this data frame instead of using stats_summary:

```{r}
Silicate_Gradient_no_Seep_new <- ggplot()+
  geom_point(data=Mean_Silicate, aes(
    x=reorder(Varari_Pin,Mean_Silicate$Mean), 
    y=Mean, 
    color="Mean"))+
  geom_point(data=BioGC.Info_no_Seep, aes(
    x=Varari_Pin, 
    y=Silicate_umolL, 
    color=Tide)) +
  ...
  ylab('Silicate (umol/L)')#+ 
  #scale_x_continuous(n.breaks=20)
Silicate_Gradient_no_Seep_new
```
Now, to compare this gradient to the change in buoyant weight
```{r}
Buoyant_Weight_on_Gradient <- ggplot()+
  geom_point(data=Mean_Silicate, aes(
    x=reorder(Varari_Pin,Mean_Silicate$Mean), 
    y=Mean, 
    color="Mean of Silicate (umol/L)"))+
  ...
  scale_color_discrete(name = "Dose", labels = c("Uncaged", "Partially Caged", "Fully Caged","Mean Silicate Concentration"))
  #theme(legend.title = element_blank())
Buoyant_Weight_on_Gradient
```
