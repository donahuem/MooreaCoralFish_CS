...
selected_columns <- c("Percent_Change", CV_columns)
...
OK so that's for a coefficient of variance hypothesis. What about mean low!
```{r PAC Dredge Mean Low 1}
PAC_model_data <- na.omit(all_data_wide %>% 
        filter(Species == "Pocillopora acuta"))
...
# Build the GLM model
PAC_mean_low_tide_model <- glmer(Percent_Change ~ . + (1 | Genotype) + (1 | Cage_Uncaged), 
                                 data = PAC_low_tide_model_data, 
                                 na.action = na.pass)

PAC_low_tide_model_dredge <- dredge(PAC_mean_low_tide_model)
#model.sel(PAC_low_tide_model_dredge)
PAC.avg_low_tide = model.avg(PAC_low_tide_model_dredge, delta < 4)
