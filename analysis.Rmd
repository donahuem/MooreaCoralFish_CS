...
```{r PAC Growth dredge for CV, warning=FALSE, message=FALSE}
...
OK so that's for a coefficient of variance hypothesis. It shows that the CV for pH, TA, and Temperature matter, but not the nutrients! Our best model includes pH, Salinity, TA, and Temperature. Here's those effects plots:
```{r effect plots}
...
What about mean low!
```{r PAC Growth Dredge Mean Low 1, warning=FALSE, message=FALSE}
PAC_model_data <- na.omit(all_data_wide %>% 
        filter(Species == "Pocillopora acuta"))
...
```
Note that there are many models with a similar Akaike weight. Note that the low tide (high SGD) values of Ammonia, Nitrate/Nitrite, NNP ratio, Phosphate, and salinity are all in most of the best models, and so they all have support from the data. Use the best model to report coefficients / effect size plots / etc. 
...
```{r PAC Symbionts CV model, message=FALSE, warning=FALSE}
PAC_model_data <- na.omit(all_data_wide %>% 
        filter(Species == "Pocillopora acuta"))
...
Megan WHAT is going on!!!! Only the most complicated model has support, but even in that, only Phosphate has significant effects?

```{r CV Symbiont Model Effects}
...
```{r PAC Symbionts Mean Low model, message=FALSE, warning=FALSE}
PAC_model_data <- na.omit(all_data_wide %>% 
        filter(Species == "Pocillopora acuta"))
...
Now, for mean low tide data, nothing is significant!!!! But only the most complicated model has support. I don't know how this means I should interpret the findings. 
