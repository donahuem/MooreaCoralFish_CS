...
```{r starting models, warning=FALSE}
...
```
```{r nyssa aim}
#do a count and summarize for genotype (to make sure no genotype is only occuring one time)
subset_ns <-all_data %>%
  filter(Parameters == "Salinity" & Species == "PAC" & Cage_Uncaged == "C")
###WE FOUND IT - YOUR FREAKIN METADATA HAS V2 AS PR21 INSTEAD OF PA21
#GO FIX THIS
all_data_mcraimignore <- all_data[!(all_data$Genotype == "PR21"), ]
all_data_mcraimignore <- all_data[!(all_data$Genotype == "PA21"), ]

all_data_mcraimignore<-all_data_mcraimignore  %>% 
                    filter(Parameters == "Salinity" & Species == "PAC" & Cage_Uncaged == "C") %>%
  droplevels()

bw_m1_caged_PAC <-lmer(Percent_Change ~ CVSeasonal + (1 | Genotype), data = all_data_mcraimignore)
anova(bw_m1_caged_PAC)
summary(bw_m1_caged_PAC)                       

subset_ns2 <- all_data_mcraimignore %>% 
                    filter(Parameters == "Salinity" & Species == "PAC" & Cage_Uncaged == "C")

summary(subset_ns2)

######## Go fix it in core metadata
#please
```
...
ggplot(gg_data, aes(x = CVSeasonal, y = Percent_Change, color=Cage_Uncaged)) +
  geom_point() +  # Add points
  geom_smooth(method = "lm")+ #ONLY ADD IN WHEN SIGNIFICANT
  labs(title = "Percent Change in Buoyant Weight vs Salinity Variability",
       x = "CV Silicate",
       y = "Percent Change")+
  facet_wrap(~Species) #WOHOO
