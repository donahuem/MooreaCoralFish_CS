...
```{r reduced multivariate}
multivariate_PAC_growth_reduced <- lmer(Percent_Change ~ poly(Minimum_Salinity_scaled,2) +
                                 poly(Low_Tide_Mean_Phosphate_umolL_scaled,2) + 
#                                  poly(shore_dist_m,2) +
                                  (1|Genotype) + (1|Cage_Uncaged), data = PAC_model_data)

#summary(multivariate_PAC_growth)
car::Anova(multivariate_PAC_growth_reduced)
plot(allEffects(multivariate_PAC_growth_reduced))
# car::vif(multivariate_PAC_growth)


```
```{r stepwise_regression, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
# Ridge/Lasso/Elastic_net
# install.packages('glmnet')
# library(glmnet)
# # Remove the Pin_Number and Species columns
# PAC_model_data <- PAC_model_data %>% dplyr::select(-Pin_Number, -Species)
# 
# # Convert Genotype and Cage_Uncaged to factors if they are not already
# PAC_model_data$Genotype <- as.factor(PAC_model_data$Genotype)
# PAC_model_data$Cage_Uncaged <- as.factor(PAC_model_data$Cage_Uncaged)
# 
# # Create dummy variables
# genotype_dummies <- model.matrix( ~ Genotype - 1, data = PAC_model_data)
# cage_uncaged_dummies <- model.matrix( ~ Cage_Uncaged - 1, data = PAC_model_data)
# 
# # Combine the dummy variables with the original data frame (excluding the original columns)
# PAC_model_data <- cbind(PAC_model_data, genotype_dummies, cage_uncaged_dummies)
# PAC_model_data <- PAC_model_data[, !names(PAC_model_data) %in% c("Genotype", "Cage_Uncaged")]
# 
# PAC_model_percent_change_df <- PAC_model_data %>%
#   dplyr::select(-FSC.Events_per_cm_2, -FSC.Events_per_g_dry_weight)
# 
...
initial_model <- lm(Percent_Change ~ ., data = PAC_model_data)
...
cor(PAC_model_data$FSC.Events_per_cm_2, PAC_model_data$Percent_Change)
ggplot(aes(x=log10(FSC.Events_per_cm_2), y = Percent_Change), data = PAC_model_data)+
  geom_point()+
  geom_smooth()
...
plot_PAC_growth_plot_data_long <- left_join(PAC_growth_plot_data_long, response_data[,c("Percent_Change", "Cage_Uncaged", "Genotype")])

plot_PAC_growth_plot_data_long$Source_Colony <- as.factor(as.integer(plot_PAC_growth_plot_data_long$Genotype))

combined_plot <- ggplot(data = plot_PAC_growth_plot_data_long, aes(x = Value, y = Percent_Change)) +
...
plot_PAC_growth_plot_data_long <- left_join(PAC_growth_plot_data_long, response_data[,c("Percent_Change", "Cage_Uncaged", "Genotype")], relationship = "many-to-many")
...
models1 <- data.frame(
   Parameter = c("sd_pH", "Minimum_Salinity", "Low_Tide_Mean_Phosphate_umolL"),
   p_value = c(0.00023, 0.00083, 0.0077),
   R2c = c(0.407, 0.270, 0.352)
 )

combined_plot <- ggplot(data = plot_PAC_growth_plot_data_long, aes(x = Value, y = Percent_Change)) +
  geom_point(aes(shape = Source_Colony, color = Cage_Uncaged)) +
  geom_smooth(data = subset(plot_PAC_growth_plot_data_long, Parameter == "sd_pH"),
                method = "lm", formula = y ~ x, color = "#265588") +
  geom_smooth(data = subset(plot_PAC_growth_plot_data_long, Parameter != "sd_pH"),
              method = "lm", formula = y ~ poly(x, 2), color = "#265588") +
  scale_color_fish_d(option = "Coryphaena_hippurus", labels = custom_labels) +
  labs(x = "", y = "Percent Change in Buoyant Weight",
       shape = "Source Colony",
       color = "Cage Treatment") +
  facet_wrap(~ Parameter, scales = "free_x", nrow = 1, labeller = labeller(Parameter = parameter_titles), strip.position= "bottom") +
  theme_linedraw() +
 theme(strip.placement = "outside",
       strip.background = element_rect(fill = "white", color = "white"),
       strip.text.x = element_text(size = 7.9, color = "black"),
       axis.text.x = element_text(hjust = 1))
```

```{r}
# Assuming combined_plot is defined as per your requirements
combined_plot <- combined_plot +
  geom_text(data = models1,
            aes(label = paste("p =", formatC(p_value, format = "e", digits = 2),
                              "\nR²c =", formatC(R2c, digits = 3)),
                x = Inf, y = -Inf), # x, y are set to Inf and -Inf to place text in lower right
            hjust = 1, vjust = -0.3, size = 3, color = "black", inherit.aes = FALSE)

print(combined_plot)

#ggsave(filename = "output/PAC_growth_multivariate_effects_plot1.png", plot = combined_plot, width = 10, height = 5)
```
...
pru_symb <- ggplot(data = PRU_plot_data, aes(x = Low_Tide_Mean_Phosphate_umolL, y = log10(FSC.Events_per_cm_2))) +
  geom_point(aes(color = Cage_Uncaged, shape = Genotype)) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2), color = "#265588") +
  scale_color_fish_d(option = "Coryphaena_hippurus",labels = custom_labels) +
  ...
  annotate("text", x = Inf, y = 3.5, 
           label = paste("p =", "2.16e-04", "\nR²c =", "0.32"),
           hjust = 1.2, vjust = .6, size = 4)

pru_symb

ggsave(filename = "output/PRU_symb_effects_plot.png", plot = pru_symb, width = 7, height = 3)
```
...
SI_join %>% 
  filter(!Placement_Code == "T0") %>% 
  filter(!HS == "host") %>% 
  ggplot(aes(y = δ15N, x = Maximum_Silicate_umolL)) +
  geom_point() +
#  geom_smooth(method = "lm") +
#  geom_text(aes(label = Pin_Number)) +
  facet_wrap(~Species)
...
SI_join %>% 
  ...
  geom_hline(yintercept = 0.2, linetype = "dashed", color = "darkblue") +
  geom_hline(yintercept = -0.2, linetype = "dashed", color = "darkblue") +
#  geom_smooth(method = "lm") +
#  geom_text(aes(label = Pin_Number)) +
  facet_wrap(~Species)
