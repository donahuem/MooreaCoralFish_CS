...
pca.max.data <- na.omit(nut_wide_no_seep[, c(maximum_columns, minimum_columns)]) #keeping V13 in this ex
...
ggplot(data)+
  geom_point(aes(x = shore_dist_m, y = Percent_Change, color = pc1))+
  scale_color_fish(option = "Coryphaena_hippurus")+
...
```{r}
bw_caged_PAC <-lmer(Percent_Change ~ shore_dist_m + pc1 + (1 | Genotype), data = all_data_pca %>% 
                    filter(Parameter == "Salinity" & 
                             Species == "Pocillopora acuta" 
                           & Cage_Uncaged == "C")) 
anova(bw_caged_PAC)
