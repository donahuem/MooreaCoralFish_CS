...
explanatory <- read.csv("data/Explanatory_variables.csv")
...
```{r all data, warning=FALSE}
#now make an all data that has both explanatory and response variables:
all_data <- left_join(response_data, explanatory, by = join_by(CowTagID))
...
```{r raw growth data plots, echo=FALSE}
...
```{r KE dredge}
all_data_wide <- left_join(response_data, nut_wide, by = join_by(CowTagID))
PAC_model_data <- na.omit(all_data_wide %>% 
        filter(Species == "Pocillopora acuta"))
PAC_big_model <- 
  glm(Percent_Change ~ CVSeasonal_Phosphate_umolL + 
        shore_dist_m + MeanSeasonal_Silicate_umolL + Minimum_pH, 
