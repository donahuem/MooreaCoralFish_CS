...
selected_columns <- c("FSC.Events_per_cm_2", "Percent_Change", "Genotype", "Cage_Uncaged", "Species","shore_dist_m",
 #                     minimum_columns, 
#                      maximum_columns,
#                      CV_columns,
#                      mean_low_columns,
...
#                      scaled_cv_columns,
                      scaled_ml_columns,
                      scaled_sd_columns,
                      "Pin_Number", "FSC.Events_per_g_dry_weight", "pulse_pc1")
...
PAC_model_scaled_data_long <- pivot_longer(
  ...
  cols = c(shore_dist_m:sd_Ammonia_umolL_scaled)) 
...
Both are better than either independently. When I add in the next two models, model averaging says it is better to include them, but if i look a this model, things aren't still significant. Not sure how to interpret this.

#### Pru growth 

Pru growth vs shore distance:
```{r}
PRU_model_data1 <- PRU_model_data1 %>%  filter(Cage_Uncaged == "C")
