...
anova_PRU <- aov(sym_FSC.Events.μL.V._normalized ~ Cage_Uncaged, data = FCM_no %>% filter(Species=="Porites_rus"))
summary(anova_PRU)

anova_PAC <- aov(sym_FSC.Events.μL.V._normalized ~ Cage_Uncaged, data = FCM_no %>% filter(Species=="Pocillopora_acuta"))
summary(anova_PAC)
```
