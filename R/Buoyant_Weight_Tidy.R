...
BwPac <- Buoyant_weight_Pac_raw %>%
  ...
  mutate(Proportional_Change = Change / T0_dry_weight.g) %>%
  mutate(Percent_Change = Proportional_Change * 100) %>%
  mutate(Change_per_day = Percent_Change/61) %>% 
  mutate(Change_Over_Area= Change / Surface.Area) %>%
  mutate(Species="PAC") %>% 
 dplyr::select(Vial_No_Only, Placement_Code, T0_dry_weight.g, T1_dry_weight.g, Change, Change_Over_Area, Percent_Change, Change_per_day, Species)
...
BwPru <- Buoyant_weight_Pru_raw %>%
  ...
  mutate(Proportional_Change = Change / T0_dry_weight.g) %>%
  mutate(Percent_Change = Proportional_Change * 100) %>%
  mutate(Change_per_day = Percent_Change/66) %>% 
  mutate(Change_Over_Area= Change / Surface_Area) %>%
  dplyr::select(Vial_No_Only, Placement_Code, T0_dry_weight.g, T1_dry_weight.g, Change, Change_Over_Area, Percent_Change, Change_per_day) %>% 
  mutate(Species="PRU")
