...
I'm going to remove unknowns, cryptic species, and eels. I'm also going to combine the stegastes that we know with ones we don't
```{r}
maxn_no_unk <- maxn %>%
  ...
  mutate(Species = ifelse(Species == "Stegastes fasciolatus", "Stegastes spp", Species)) %>% 
  mutate(Species = ifelse(Species == "Stegastes sp", "Stegastes spp", Species))
```
...
But now, I'm going to comment it out:
```{r}
# # Identify extreme observations I am using the 99% quantile instead because my data is smaller
# extreme_threshold <- quantile(subset_maxn_combined_life_stages$MaxN, probs = 0.999) 
# 
...
#   pull(Species) %>%
#   unique()
# #it's literally just one obvs of manini
# 
...
```{r}
subset_maxn_combined_life_stages$Block <- paste0(subset_maxn_combined_life_stages$On.Adjacent, " ", subset_maxn_combined_life_stages$Block) #I want to make sure that factor levels are different for each combo of on/adjacent and block so that I don't accidentially combine blocks 
```
...
```{r make fish tidy}
fish_tidy <- subset_maxn_combined_life_stages
```
...
Fish2 <-wide_maxn_df[c(4:ncol(wide_maxn_df))]
...
##max is a transformation that standardizes every fish to their max n
...
Species_only <-wide_maxn_df[c(4:ncol(wide_maxn_df))]
...
```{r}
anova_result <- anova(cap, by = "margin")
anova_result
total_ss <- sum(anova_result$`SumOfSqs`, na.rm = TRUE)  # includes residuals if present
prop_explained <- anova_result$`SumOfSqs` / total_ss
prop_explained
```

Interestingly, all the axes are coming up as significant contributors to this variation.

But, what about the r2?
```{r}
RsquareAdj(cap)
```
