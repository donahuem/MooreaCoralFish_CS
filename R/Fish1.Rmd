...
wide_maxn_df <- pivot_wider(reef_pins_fish_tidy, 
                             id_cols = c("Pin", "Block"),
                             names_from = Species, 
                             values_from = MaxN)%>% 
  mutate_all(~ifelse(is.na(.), 0, .))
...
ggplot(maxn_avg1, aes(y = Species_order, x = avg_MaxN, fill = Family)) +
  ...
  theme_minimal() +
#  geom_text(aes(label = Family), hjust = -0.2, size = 3, color = "black") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), # Rotate x-axis labels for readability
        legend.position = "left") + # Remove the legend
...
UPDATE, after meeting with Megan
Beacuse this is really one site, and we're thinking about fish movement patterns, we're doing a species maximum standardization.
