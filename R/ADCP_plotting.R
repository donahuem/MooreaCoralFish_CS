...
d.adcp <- read.csv("https://raw.githubusercontent.com/njsilbiger/MooreaSGD_site-selection/main/Data/ADCP/ADCP_all.csv")
d.adcp <-d.adcp %>% 
...
#WET SEASON
  v2<-  plot.windrose(spd = d.adcp$Speed[d.adcp$ID=="220320"],
    ...
    labs(x = "",
         y = "")+
    theme_minimal()+
    #scale_y_continuous(limits = c(0,1500))+ #adding in this line somehow destroyed the highest data point?
    theme(axis.text = element_blank(),
          axis.ticks = element_blank(),
          panel.border = element_blank(),
          panel.grid = element_line(color = "grey"),
...
v1+plot_layout()
adcp <- v2+plot_layout(guides = "collect")
  
ggsave(filename = "output/varari_wet_windrose.png", adcp, height = 5, width = 5)
# #Cabral
...
# meta <- read.csv("https://raw.githubusercontent.com/njsilbiger/MooreaSGD_site-selection/main/Data/ADCP_metadata.csv")
# #depth bins 
